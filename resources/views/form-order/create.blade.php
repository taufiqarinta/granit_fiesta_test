            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('form-order.store') }}" method="POST" id="formOrder">
                        ...
                        <!-- Input hidden untuk menyimpan data lama -->
                        <input type="hidden" name="pic_old" id="pic_old" value="{{ old('pic_old') }}">
                        <input type="hidden" name="nomor_pic_old" id="nomor_pic_old" value="{{ old('nomor_pic_old') }}">
                        <input type="hidden" name="source" value="create-page">
                                    ...
                                    <table class="min-w-full border-2 border-gray-800">
                                        <thead class="bg-gray-100">
                                            <tr>
                                                ...
                                                <th class="px-2 sm:px-4 py-3 text-center text-xs sm:text-sm font-bold text-gray-900 uppercase border-2 border-gray-800 whitespace-nowrap hidden">KUPON/PAKET</th>
                                                ...
                                                <th class="px-2 sm:px-4 py-3 text-center text-xs sm:text-sm font-bold text-gray-900 uppercase border-2 border-gray-800 whitespace-nowrap hidden">TOTAL KUPON</th>
                                            </tr>
                                        </thead>
                                        <tbody class="bg-white divide-y divide-gray-800">
                                            @foreach($masterTargets as $index => $target)
                                                <tr>
                                                    ...
                                                    <td class="px-2 sm:px-4 py-4 text-center border-2 border-gray-800 font-bold text-sm whitespace-nowrap hidden">
                                                    ...
                                                    <td class="px-2 sm:px-4 py-4 text-center border-2 border-gray-800 font-bold whitespace-nowrap">
                                                        <span id="total_point_{{ $target->id }}" class="text-blue-600 text-sm sm:text-lg transition-all duration-300">0</span>
                                                    </td>
                                                    
                                                    <td class="px-2 sm:px-4 py-4 text-center border-2 border-gray-800 font-bold whitespace-nowrap">
                                                        <span id="total_kupon_{{ $target->id }}" class="text-purple-600 text-sm sm:text-lg transition-all duration-300 hidden">0</span>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                        <tfoot class="bg-gray-50">
                                            <tr>
                                                ...
                                                <td class="px-2 sm:px-4 py-4 text-center font-bold border-2 border-gray-800 whitespace-nowrap">
                                                    <span id="grandTotalPoint" class="text-green-600 text-base sm:text-xl transition-all duration-500">0</span>
                                                </td>
                                                <td class="px-2 sm:px-4 py-4 text-center font-bold border-2 border-gray-800 whitespace-nowrap hidden">
                                                    <span id="grandTotalKupon" class="text-green-600 text-base sm:text-xl transition-all duration-500">0</span>
                                                </td>
                                            </tr>
                                            <tr class = "hidden">
                                                <td colspan="4" class="px-2 sm:px-4 py-4 text-right font-bold text-sm sm:text-lg border-2 border-gray-800">
                                                    TOTAL VOUCHER
                                                </td>
                                                <td colspan="2" class="px-2 sm:px-4 py-4 text-center font-bold border-2 border-gray-800 whitespace-nowrap">
                                                    <span id="totalVoucher" class="text-red-600 text-base sm:text-xl transition-all duration-500">0</span>
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#lokasi_event').on('change', function() {
                ...
                // Reset filter jika lokasi dipilih ulang
                if (selectedLokasi) {
                    // Filter toko berdasarkan lokasi event yang dipilih
                    filterTokoByLokasi(selectedLokasi);
                    
                    // Check duplicate jika sudah ada toko yang dipilih
            ...
            // Function untuk filter toko berdasarkan lokasi event
            function filterTokoByLokasi(lokasiEvent) {
                const $selectToko = $('#nama_toko');
                const currentValue = $selectToko.val();
                
                // Reset select toko
                $selectToko.empty();
                $selectToko.append('<option value="">Pilih Toko</option>');
                
                let hasMatchingToko = false;
                
                // Filter dan tambahkan hanya toko yang sesuai dengan lokasi event
                @foreach($daftarTokoUnique ?? [] as $toko)
                    @php
                        $tokoLokasi = $toko->lokasi_event;
                    @endphp
                    if ("{{ $tokoLokasi }}" === lokasiEvent || lokasiEvent === '') {
                        $selectToko.append(
                            '<option value="{{ $toko->id }}" ' +
                            'data-kota="{{ $toko->kota }}" ' +
                            'data-pic="{{ $toko->pic }}" ' +
                        ...
                        hasMatchingToko = true;
                    }
                @endforeach
                
                if (!hasMatchingToko) {
                    $selectToko.append('<option value="" disabled>Tidak ada toko untuk lokasi ini</option>');
                }
                ...
                $selectToko.select2({
                    ...
                    language: {
                        noResults: function() {
                            return "Tidak ada toko untuk lokasi ini";
                        }
                    }
                });
                
                // Set nilai lama jika masih valid
                if (currentValue && hasMatchingToko) {
                    $selectToko.val(currentValue).trigger('change');
                }
            }
            ...
            $('#nama_toko').on('change', function() {
                ...
                // Tidak reset lokasi_event karena sudah dipilih di dropdown
                ...
                if (tokoId && tokoData[tokoId]) {
                    ...
                    // Check for duplicate setelah data terisi
                    setTimeout(() => {
                        @if($user->department == 'SLS')
                        const agenId = $('#nama_agen').val();
                        if (agenId) {
            ...
            function loadNamaSales(agenData, tokoId) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        lokasi_event: $('#lokasi_event').val() // Tambahkan lokasi event
                    },
            ...
            @if(old('lokasi_event'))
                $('#lokasi_event').val('{{ old('lokasi_event') }}').trigger('change');
            @endif
