...
function doLookupAgen(kode) {
    ...
    return $.get('{{ url('/api/lookup-agen-by-kode') }}', { kode_agen: kode })
        ...
        .fail(function() { alertErr('Gagal melakukan lookup agen'); });
}
...
$('#scanForm').on('submit', function(e) {
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.success) {
                window.location.href = res.redirect_url || ('{{ route('form-order.success') }}?kode_unik=' +
                    encodeURIComponent(res.kode_unik || '') + '&jumlah_voucher=' +
                    encodeURIComponent(res.jumlah_voucher || 0));
            } else {
