...
<body>
    ...
    <script>
        ...
        function autoSelectUangTunai() {
            ...
            // Cari option yang mengandung "Uang Tunai"
            for (let i = 0; i < doorprizeSelect.options.length; i++) {
                const option = doorprizeSelect.options[i];
                if (option.text.includes('Uang Tunai')) {
                    console.log('Found Uang Tunai option:', option);
                    
                    // Set value select
                    doorprizeSelect.value = option.value;
                    
                    // Cari elemen doorprize yang sesuai dan beri class selected
                    const doorprizeItems = document.querySelectorAll('.doorprize-item');
                    doorprizeItems.forEach(item => {
                        // Hapus seleksi sebelumnya
                        item.classList.remove('selected');
                        // Tandai yang dipilih jika doorprizeId match
                        if (item.dataset.doorprizeId === option.value) {
                            item.classList.add('selected');
                            // console.log('Doorprize item selected:', item);
                        }
                    });
                    ...
                    // console.log('Uang Tunai berhasil dipilih');
                    break;
                }
            }
        }
        ...
        // Fungsi untuk mereset informasi voucher menjadi "*****"
