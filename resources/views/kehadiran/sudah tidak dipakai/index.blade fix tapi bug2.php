<x-app-layout>
        ...
        <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6">
                    ...
                    <div class="table-container" style="width: 100%; height: 70vh; overflow: auto; border: 1px solid #e5e7eb; border-radius: 8px;">
                        <table id="tabelDaftarToko" style="min-width: 1300px; width: 100%; border-collapse: collapse; background: white;">
                            <thead style="background-color: #f9fafb; position: sticky; top: 0; z-index: 20;">
                                <tr>
                                    <th style="border: 1px solid #e5e7eb; padding: 12px; text-align: center; font-size: 0.75rem; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; position: sticky; left: 0; background: #f9fafb; z-index: 30; min-width: 60px;">No</th>
                                    ...
                                    <th style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.75rem; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 120px;">Kode</th>
                                    <th style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.75rem; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 200px;">Nama</th>
                            ...
                            <tbody>
                                @foreach($gabunganData as $index => $data)
                                    <tr id="row-{{ $data['id'] }}" style="{{ $data['hadir'] ? 'background-color: #f0fdf4;' : '' }} transition: background-color 0.15s ease-in-out;">
                                        <!-- Nomor Urut - Sticky -->
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: center; font-size: 0.875rem; color: #111827; font-weight: 500; position: sticky; left: 0; background: inherit; z-index: 10;">
                                            {{ $index + 1 }}
                                        </td>
                                        
                                        <!-- Tipe (Toko/Agen) -->
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: center; font-size: 0.875rem; color: #111827; white-space: nowrap;">
                                            <span class="px-2 py-1 rounded-full text-xs font-bold {{ $data['type'] == 'toko' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }}">
                                                {{ $data['type'] == 'toko' ? 'TOKO' : 'AGEN' }}
                                            </span>
                                        </td>
                                        ...
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827;">
                                            <div style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $data['nama_toko'] }}">
                                                {{ $data['nama_toko'] }}
                                            </div>
                                        </td>
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827;">
                                            <div style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $data['pic'] }}">
                                                {{ $data['pic'] }}
                                            </div>
                                        </td>
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827; white-space: nowrap;">
                                        ...
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827;">
                                            <div style="max-width: 250px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $data['alamat'] }}">
                                                {{ $data['alamat'] }}
                                            </div>
                                        </td>
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827;">
                                            <div style="max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $wilayahData[$data['id']]['provinsi'] }}">
                                                {{ $wilayahData[$data['id']]['provinsi'] }}
                                            </div>
                                        </td>
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827;">
                                            <div style="max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $wilayahData[$data['id']]['kota'] }}">
                                                {{ $wilayahData[$data['id']]['kota'] }}
                                            </div>
                                        </td>
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827; white-space: nowrap;">
                                            {{ $data['kode_agen'] }}
                                        ...
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827;">
                                            <div style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $data['nama_agen'] }}">
                                                {{ $data['nama_agen'] }}
                                            </div>
                                        </td>
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: left; font-size: 0.875rem; color: #111827;">
                                            <div style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $data['nama_sales'] }}">
                                                {{ $data['nama_sales'] }}
                                            </div>
                                        </td>
                                        ...
                                        <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center; background: inherit;">
                                            <input type="number" 
                                                   id="jumlah-kehadiran-{{ $data['id'] }}"
                                                   value="{{ $data['jumlah_kehadiran'] }}"
                                                   min="0"
                                                   onchange="ubahJumlahKehadiran('{{ $data['id'] }}', this.value)"
                                        ...
                                        <!-- Kolom Hadir - Sticky -->
                                        <td style="border: 1px solid #e5e7eb; padding: 12px; text-align: center; position: sticky; right: 0; background: inherit; z-index: 10;">
                                            <label style="display: inline-flex; align-items: center; cursor: pointer;">
                                                <input type="checkbox" 
                                                       {{ $data['hadir'] ? 'checked' : '' }}
                                                       onchange="ubahHadir('{{ $data['id'] }}', this.checked)"
                                ...
                                @if(count($gabunganData) == 0)
                                    <tr>
                                        <td colspan="14" style="border: 1px solid #e5e7eb; padding: 32px; text-align: center; font-size: 0.875rem; color: #6b7280;">
                    ...
                    <div style="margin-top: 16px; font-size: 0.75rem; color: #6b7280; text-align: center;">
                        <span>📱 Geser tabel ke samping/bawah untuk melihat data lainnya</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <style>
        ...
        /* Pastikan z-index untuk header lebih tinggi */
        table thead th {
            position: sticky;
            top: 0;
            z-index: 25;
