<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6">
                    ...
                    <form method="POST" action="{{ route('daftaragen.update', $agen->id) }}" id="agenForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mt-4 md:mt-6">
                            ...
                            <textarea name="alamat" 
                                ...
                                required>{{ old('alamat', $agen->alamat) }}</textarea>
                            @error('alamat')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div>
                                ...
                                <select name="provinsi" 
                                    ...
                                    @foreach ($provinsis as $prov)
                                        <option value="{{ $prov->kode }}" {{ old('provinsi', $agen->provinsi) == $prov->kode ? 'selected' : '' }}>
                                            {{ $prov->nama }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-4 md:mt-6">
                            ...
                            <div>
                                ...
                                <input type="text" 
                                    name="nomor_pic" 
                                    id="nomor_pic" 
                                    value="{{ old('nomor_pic', $agen->nomor_pic) }}"
                                ...
                                <select name="lokasi_event" 
                                    ...
                                    @foreach($lokasiEvents as $lokasi)
                                        <option value="{{ $lokasi->nama_lokasi }}" 
                                            {{ old('lokasi_event', $agen->lokasi_event) == $lokasi->nama_lokasi ? 'selected' : '' }}>
                                            {{ $lokasi->nama_lokasi }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="status" 
                                    ...
                                    required>
                                    <option value="1" {{ old('status', $agen->status) == 1 ? 'selected' : '' }}>Aktif</option>
                                    <option value="0" {{ old('status', $agen->status) == 0 ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                    ...
                                    @foreach($merks as $merk)
                                        <div class="flex items-start p-3 bg-white rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                                            <input type="checkbox" 
                                                ...
                                                {{ in_array($merk->id, old('merks', $selectedMerks)) ? 'checked' : '' }}>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const selectedKota = '{{ old('kota', $agen->kota) }}';
                    ...
                    fetch(`/kabupaten?kode_provinsi=${provinsiKode}`)
                        ...
                        .then(data => {
                            ...
                            data.forEach(item => {
                                ...
                                // Set selected jika sesuai dengan data lama
                                if (item.kode === selectedKota) {
        ...
        // Validasi form untuk memastikan minimal satu merk dipilih
        document.getElementById('agenForm').addEventListener('submit', function(e) {
            const merkCheckboxes = document.querySelectorAll('input[name="merks[]"]:checked');
            
            if (merkCheckboxes.length === 0) {
                e.preventDefault();
                alert('Pilih minimal satu merk');
                return false;
            }
        });

        // Optional: Select all functionality jika diperlukan
        function selectAllMerks(selectAll) {
            const checkboxes = document.querySelectorAll('input[name="merks[]"]');
