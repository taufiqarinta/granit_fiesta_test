...
class KehadiranExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths
{
    ...
    {
        // Ambil data dari kedua tabel dengan urutan waktu kehadiran ascending (terawal ke terakhir)
        $daftarTokos = DaftarToko::where('lokasi_event', $this->lokasiEvent)
                        ->where('status', 1)
                        ->orderBy('waktu_kehadiran', 'asc') // Ubah ke asc untuk terawal
                        ->get();
        ...
        // Urutkan berdasarkan waktu kehadiran terawal (ascending)
        return $gabunganData->sortBy(function($item) {
            return $item->waktu_kehadiran_timestamp;
        })->values();
    }

    /**
     * Convert waktu kehadiran string ke timestamp untuk sorting
     */
    private function convertToTimestamp($waktuKehadiran)
    {
        if (empty($waktuKehadiran)) {
            return PHP_INT_MAX; // Return nilai besar untuk data tanpa waktu
        }
        
        try {
            return strtotime($waktuKehadiran);
        } catch (\Exception $e) {
            return PHP_INT_MAX;
        }
    }
