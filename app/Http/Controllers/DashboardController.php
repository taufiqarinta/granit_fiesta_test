...
class DashboardController extends Controller
{
    public function index()
    {
        try {
            ...
            // ----------------------------
            // 1. Jumlah Toko (unique by nama_toko, pic, nomor_pic, kota, lokasi_event)
            // ----------------------------
            $sub = DB::table('daftar_toko')
                ...
                ->where('lokasi_event', '!=', '')
                ->where('status', 1)
                ->groupBy('lokasi_event', 'nama_toko', 'pic', 'nomor_pic', 'kota');

            $jumlahTokoRaw = DB::query()
                ->fromSub($sub, 'uniq_toko')
            ...
            // 3. Jumlah Kehadiran Toko (unique by nama_toko, pic, nomor_pic, kota, lokasi_event)
            $subKehadiran = DB::table('daftar_toko')
                ->select('lokasi_event', 'nama_toko', 'pic', 'nomor_pic', 'kota')
                ->where('hadir', 1)
                ->where('status', 1)
                ->whereNotNull('lokasi_event')
            ...
            $kehadiranTokoRaw = DB::query()
                ->fromSub($subKehadiran, 'uniq_hadir_toko')
                ->select('lokasi_event', DB::raw('COUNT(*) as total'))
                ->groupBy('lokasi_event')
                ->pluck('total', 'lokasi_event');
            ...
            foreach ($masterLokasi as $lokasi) {
                ...
                // Jumlah Toko (dari hasil subquery uniq)
